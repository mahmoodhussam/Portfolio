...
if (bgOption !== null) {
  ...
  document
    .querySelectorAll(".random-background-container span")
    .forEach((e) => {
      e.classList.remove("active");
    });
  if (backgroundRandom === true) {
    document
      .querySelector(".random-background-container .yes")
      .classList.add("active");
  } else {
    document
      .querySelector(".random-background-container .no")
      .classList.add("active");
  }
}
...
window.addEventListener("scroll", () => {
  ...
  if (
    windowScrollUp >
    skillsOffsetTop + skillsOffsetHight - windowHeight - 250
  ) {
...
const projectTabs = document.querySelectorAll(".tabs-container .project-tab");
  ...
  tab.addEventListener("click", (e) => {
    ...
    projects.forEach((project) => {
      if (e.target.dataset.value == "all") {
        ...
        if (e.target.dataset.value == project.dataset.project) {
...
ourGallery.forEach((img) => {
  img.addEventListener("click", (e) => {
    ...
    imageLink.href = img.dataset.link;
    imageLink.classList = "image-info-link";
    imageLink.textContent = "Check it here";
    imageLink.target = "_blank";
    imageInfo.className = "image-info";
    let imageHeadingContent = document.createTextNode(img.alt);
    let imageInfoContent = document.createTextNode(img.dataset.info);
    imageHeading.appendChild(imageHeadingContent);
    imageInfo.appendChild(imageInfoContent);
    ...
    popupBox.appendChild(imageInfo);
    console.log(img.src);
